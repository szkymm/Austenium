...
- The version string structure settled on 2026-09-13 lives in AGENT.md section 1.2.

### 0.0.1, the coverage measurement and the platform floor (released 2026-09-16)

- Scope: this cycle changes no Java source and no JSON resource; only the three version ranges in `mods.toml` move. The version answers one question: which platforms can be built and run from this frozen source.
- Method: 735 Forge builds from 1.20 through 26.2 were enumerated from the official maven metadata and reduced to 40 "MC version x loader line" targets; each line's earliest build was compiled against the frozen source (`tools/coverage_build.py`, four passes, resume support, a stall watchdog). Every passing platform then booted a real Forge server carrying the built jar (`tools/coverage_server_probe.py`) and was graded on reaching `Done`, printing the mod initialisation line and showing no linkage error.
- Result: only Minecraft 1.20.1 with the Forge 47.4 line compiles and runs. The package count is 1, the endpoints are `[47.4.0, 47.4.23]`, the compile baseline is 47.4.0 and the platform string is `0L.5.0`. Forge 47.0 to 47.3 and Minecraft 1.20 fail on missing APIs (`ResourceLocation.fromNamespaceAndPath` and `withDefaultNamespace` exist only in the 47.4 line's patched jar, `MobSpawnEvent.PositionCheck` arrived with 47.1), Minecraft 1.20.2 to 1.20.4 fail with 74 to 100 compile errors, and 1.20.6 and later cannot be built at all by ForgeGradle 6.0.x because the root project has no `reobfJar` task.
- The measurement corrected two over-claims in the shipped metadata: the Forge floor moved to `[47.4.0,)` and the Minecraft range narrowed to `[1.20.1,1.20.2)`. Identical boundaries are invalid for Forge's range parser, hence the half-open form.
- The negative controls matter as much as the positive ones: the shipped jar was refused on Forge 47.3.0 and on Minecraft 1.20, and with both gates opened in a throwaway copy of the jar it still failed to load on 1.20, which shows the refusal protects users from a mod that would not have worked.
- Toolchain traps met along the way: this network resolves the Forge and Mojang hosts to the router, so the build configuration points the daemon at the local proxy `127.0.0.1:10090` and sets socket timeouts; the Forge installer needs the vanilla server jar placed by hand from the Gradle cache; `[MBB]` inside a shell glob is a character class, so old jars must be deleted by name.
