 ...
 *
 * MODULE: com.mbb.austenium.content.menu.IronGridMenu
 * TYPE: Java Source
...
/**
 * IronGridMenu CLASS IS CORE PART OF [MBB] AUSTENIUM IronGridMenu.java.
 *
 * com.mbb.austenium.content.menu.IronGridMenu:
 *     Container menu with a custom columns x rows grid (e.g. 10x4 = 40 slots).
 */
public class IronGridMenu extends AbstractContainerMenu {
    ...
    public IronGridMenu(MenuType<?> type, int containerId, Inventory inventory, Container container, int rows, int cols) {
        ...
        // Align with the themed background: the player block is centered in the window.
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            if (index < containerSlots) {
                if (!this.moveItemStackTo(item, containerSlots, this.slots.size(), true)) {
